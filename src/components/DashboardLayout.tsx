...
export function DashboardLayout({
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      {/* ================= DESKTOP SIDEBAR ================= */}
      <aside
        className={`hidden md:flex flex-col bg-white/70 backdrop-blur-xl border-r border-gray-200 transition-all duration-300 ${
          collapsed ? "w-20" : "w-64"
        }`}
      >
        {/* Logo + Inline Toggle */}
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <button
              ...
              className={`relative inline-flex h-5 w-10 items-center rounded-full transition ${
                collapsed
                  ? "bg-gray-300"
                  : "bg-gradient-to-r from-[#3B82F6] to-[#0a1c4f]"
              }`}
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition ${
                  collapsed ? "translate-x-1" : "translate-x-5"
                }`}
              />
            </button>
          </div>
        ...
        <div className="p-3 border-t">
          <button
            ...
            className={`w-full flex items-center ${
              collapsed ? "justify-center" : "gap-3"
            } px-4 py-2 rounded-lg text-sm text-red-600 hover:bg-red-50 transition`}
          >
